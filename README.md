...
> **[UI Mockups on Figma](https://primer-fire-61312468.figma.site/)**
...
* **Sprint 4 (Milestone C4):** Production polish – README updates, Docker fixes, Axios auth alignment, and data isolation fixes.
...
$$
\text{Progress \%} = \left( \frac{\text{pagesRead}}{\text{pagesTotal}} \right) \times 100
$$
...
* **User.js:** Defines the schema for authentication, storing emails and hashed passwords.
...
* **User Entity:** `_id`, `email`, `password`.
* **Book Entity:** `_id`, `title`, `author`, `pagesRead`, `pagesTotal`, `status`, `owner`.
```mermaid
erDiagram
    USERS ||--o{ BOOKS : owns
    USERS {
...
```
Below is the **updated** version of your "Auth Middleware" section, reflecting the new code that properly handles `Bearer` tokens (making it RFC 6750 compliant and more robust). You can replace the old code and explanation in your README with this.

---

### Detailed Functionality Spotlight: Auth Middleware 

To demonstrate good programming practices and adherence to industry standards, I implemented a **Authorization Middleware** that follows the RFC 6750 Bearer Token specification. This security layer ensures that only authenticated users can access protected endpoints (e.g., adding, editing, or deleting books).

**Development Process:**

1. **Requirement:**  
   Protect private endpoints so that only logged-in users can access their own data (e.g., books, notes). The middleware must support the standard `Authorization: Bearer <token>` header format while remaining backward‑compatible with raw tokens (if any legacy client sends them).

2. **Logic design:**  
   The middleware acts as a **gatekeeper**. It:
   - Reads the `Authorization` header.
   - Handles both pure tokens and the `Bearer <token>` format.
   - Verifies the token using the server’s secret key.
   - Attaches the decoded user ID to the request object (`req.user`), so downstream route handlers know exactly which user is making the request.
...
module.exports = (req, res, next) => {
  ...
  // 1. If no Authorization header is present, block immediately
  if (!authHeader) return res.status(401).json({ error: "No token provided" });

  // 2. Extract token – supports both "Bearer <token>" and raw "<token>"
  ...
  try {
    // 3. Verify the token using the secret environment variable
    ...
    // 6. Verification failed (expired, tampered, or malformed token)
...
4. **Testing:**  
   Verified the middleware using **Postman** with three scenarios:  
   - No `Authorization` header → `401 No token provided`.  
   - Header with `Bearer validToken` → request proceeds, `req.user` set.  
   - Header with raw `validToken` (no `Bearer`) → also works.  
   - Header with expired or invalid token → `401 Invalid or expired token`.

5. **Edge Cases Handled:**  
   - Missing `Authorization` header.  
   - `Bearer` prefix presence or absence.  
   - Token that is malformed, expired, or signed with the wrong secret.  
   - Missing `JWT_SECRET` environment variable (will cause a clear error on server start if checked elsewhere).
...
sequenceDiagram
    ...
    alt Credentials Valid
        ...
        U->>BE: Request to protected endpoint (e.g., GET /api/books)
        Note over BE: Auth middleware extracts & verifies token
        BE-->>U: HTTP 200 (Protected data)
...
**Why This Version Is Better (for your README / portfolio):**

- **Standards‑compliant:** Accepts the official `Bearer` scheme (RFC 6750) – works seamlessly with all modern HTTP clients, API gateways, and Postman’s “Bearer Token” mode.
- **Robust:** Automatically strips the `Bearer` prefix, so even if a client sends an incorrect format, the middleware remains tolerant.
- **Clear error messages:** Distinguishes between missing token and invalid/expired token.
- **Secure:** Uses environment variables for the secret, does not leak token validation details.

You can now confidently present this as an example of **production‑grade authentication middleware** in your documentation.
...
2.  **Orchestration:** Use `docker-compose up -d --build` to run the services in detached mode.
3. **Deployment:** The Dockerized application can be deployed to platforms that support containerized services.
...
4. **Filtering:** Use the navigation tabs to switch between "All", "Reading", and "Finished" views.
